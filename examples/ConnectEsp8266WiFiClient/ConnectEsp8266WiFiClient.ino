...
void initializeESP8266()
{
  ...
  if (test != true)
  {
    LOG_PRINTFLN("Error talking to ESP8266.");
    errorLoop(test);
  }
  esp8266.setMux(0);
  LOG_PRINTFLN("ESP8266 Shield Present");
}
...
void connectESP8266()
{
  ...
  { // If it's not in station mode.
    ...
    if (retVal < 0)
    {
      LOG_PRINTFLN("Error setting mode.");
      errorLoop(retVal);
    }
  }
  LOG_PRINTFLN("Mode set to station");
  ...
  if (retVal <= 0)
  {
    ...
    if (retVal < 0)
    {
      LOG_PRINTFLN("Error to connecting");
      errorLoop(retVal);
    }
  }
}
...
void displayConnectInfo()
{
  ...
  LOG_PRINTFLN("Connected to WiFi: %s", connectedSSID);
...
void setup() {
  ...
  MqttClient::Network * mqttNetwork = new MqttClient::NetworkClientImpl<ESP8266Client>(client, *mqttSystem);
...
void loop() {
  ...
  if (!mqtt->isConnected()) {
    ...
    LOG_PRINTFLN("Connecting to %s", mqttServer);
    client.connect(mqttServer, mqttPort, 1000);    
    if (!client.connected()) {
