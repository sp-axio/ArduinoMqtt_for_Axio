...
int ESP8266Mbedtls::connect(IPAddress ip, uint16_t port)
{
	return connect(ip, port, 0);
}

int ESP8266Mbedtls::connect(const char *host, uint16_t port)
{
	return connect(host, port, 0);
}

int ESP8266Mbedtls::connect(String host, uint16_t port, uint32_t keepAlive)
{
	return connect(host.c_str(), port, keepAlive);
}
	
int ESP8266Mbedtls::connect(IPAddress ip, uint16_t port, uint32_t keepAlive) 
{
	char ipAddress[16];
